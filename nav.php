 ...
 * Site navigation.
 *
 * Rendered on wp_body_open rather than printed inside header.php, because
 * Elementor's page templates (Canvas in particular) never load header.php —
 * which is exactly how the menu vanished on the pages built with Elementor.
 * wp_body_open fires in every template, Elementor's included, so hooking here
 * is what keeps one nav on every page of the site.
 *
 * The static guard means it can only ever print once per request, however many
 * templates fire the hook.
 *
...
add_action( 'wp_body_open', 'easylot_render_nav', 5 );
